        ...
        <div class="auth-wrapper animate-on-scroll fade-up">
            <div class="auth-card">
                ...
                <div id="login-password-form">
                    ...
                    <button class="btn btn-outline-primary btn-block" id="show-otp-login" type="button"><i class="fas fa-mobile-alt"></i> Login with OTP</button>
                </div>
                ...
                <div id="login-otp-form" class="hidden">
                    <form id="otp-form" class="form">
                        <div class="form-group" id="otp-phone-group">
                            ...
                            <button type="button" class="btn btn-primary btn-block mt-2" id="send-otp-btn"><i class="fas fa-paper-plane"></i> Send OTP</button>
                        </div>
                        <div class="form-group hidden" id="otp-input-group">
                            ...
                            <button type="button" class="btn btn-primary btn-block mt-2" id="verify-otp-btn"><i class="fas fa-check"></i> Verify & Login</button>
                            <button type="button" class="btn btn-link btn-block mt-2" id="resend-otp-btn" disabled>Resend OTP <span id="otp-countdown">(60s)</span></button>
                        </div>
                    </form>
                    <button class="btn btn-link btn-block mt-2" id="show-password-login" type="button"><i class="fas fa-arrow-left"></i> Back to password login</button>
                </div>

                <div class="auth-footer">
                    <p>Don't have an account? <a href="<?= SITE_URL ?>/pages/register.php<?= $next !== '' ? '?next=' . urlencode($next) : '' ?>">Register Now</a></p>
                </div>
            </div>
        </div>
    </div>
...
$(document).ready(function() {
    ...
    function goAfterLogin() {
        const target = redirectAfterLogin || SITE_URL + '/';
    ...
    $('#send-otp-btn').on('click', function() {
        ...
        $.ajax({
            url: SITE_URL + '/api/auth/send-otp.php', method: 'POST', contentType: 'application/json',
            data: JSON.stringify({ phone: phone }),
            success: function(res) {
                if (res.success) { showToast('OTP sent to your mobile number.', 'success'); showOtpStep(); }
                else showToast(res.message || 'Unable to send OTP', 'error');
            },
            error: function(xhr) { const res = xhr.responseJSON || {}; showToast(res.message || 'Unable to send OTP. Please try again.', 'error'); },
            complete: function() { $btn.prop('disabled', false).html('<i class="fas fa-paper-plane"></i> Send OTP'); }
        });
    });

    $('#resend-otp-btn').on('click', function() { $('#send-otp-btn').trigger('click'); });

    $('#verify-otp-btn').on('click', function() {
        const phone = $('[name="otp_phone"]').val().replace(/\D/g, '');
        const otp = $('[name="otp_code"]').val().replace(/\D/g, '');
        $('[name="otp_code"]').val(otp);
        if (!/^[6-9]\d{9}$/.test(phone)) { showToast('Enter a valid phone number', 'error'); return; }
        if (!/^\d{6}$/.test(otp)) { showToast('Enter the 6-digit OTP', 'error'); return; }

        const $btn = $(this);
        $btn.prop('disabled', true).html('<i class="fas fa-spinner fa-spin"></i> Verifying...');
        $.ajax({
            url: SITE_URL + '/api/auth/verify-otp.php', method: 'POST', contentType: 'application/json',
            ...
            success: function(res) {
                if (res.success) {
                    ...
                    setTimeout(goAfterLogin, 700);
                } else showToast(res.message || 'OTP verification failed', 'error');
            },
            error: function(xhr) { const res = xhr.responseJSON || {}; showToast(res.message || 'OTP verification failed', 'error'); },
            complete: function() { $btn.prop('disabled', false).html('<i class="fas fa-check"></i> Verify & Login'); }
        });
    });
});
</script>
