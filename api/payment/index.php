...
function cashfreeRequest($endpoint, $payload = null, $method = 'POST') {
    ...
    if (!is_array($decoded)) return ['error' => true, 'http_code' => $httpCode];
...
function getRequestHeader($names) {
    ...
    foreach ($names as $wanted) {
        foreach ($headers as $name => $value) {
            if (strtolower($name) === strtolower($wanted)) return trim($value);
        }
    }
    return '';
}
...
/** Restore stock exactly once when a pending online order becomes failed/cancelled. */
function restoreReservedOrderStock($orderId) {
    global $db;
    ...
    foreach ($items as $item) {
        $db->update(
            ...
            [(int)$item['quantity'], (int)$item['product_id']], 'ii'
        );
    }
}
...
if ($action === 'create-order') {
    ...
    $order = $db->fetchOne("SELECT * FROM orders WHERE id = ? AND user_id = ? AND payment_method = 'online'", [$orderId, $user['id']], 'ii');
    if (!$order) jsonResponse(['success' => false, 'message' => 'Online payment order not found'], 404);
    if ($order['payment_status'] === 'paid') jsonResponse(['success' => false, 'message' => 'Order is already paid'], 409);
    ...
    if (isset($res['http_code']) && in_array($res['http_code'], [200, 201], true) && !empty($res['payment_session_id'])) {
        $db->update("UPDATE orders SET cf_order_id = ?, payment_session_id = ? WHERE id = ? AND payment_status = 'pending'", [$cfOrderId, $res['payment_session_id'], $orderId], 'ssi');
        jsonResponse([
            'success' => true,
...
if ($action === 'verify') {
    ...
    if (empty($cfOrderId) || $orderId <= 0) jsonResponse(['success' => false, 'message' => 'Order details missing'], 400);

    $order = $db->fetchOne("SELECT * FROM orders WHERE id = ? AND user_id = ? AND payment_method = 'online'", [$orderId, $user['id']], 'ii');
    if (!$order) jsonResponse(['success' => false, 'message' => 'Order not found'], 404);
    if (empty($order['cf_order_id']) || !hash_equals((string)$order['cf_order_id'], (string)$cfOrderId)) jsonResponse(['success' => false, 'message' => 'Payment order mismatch'], 400);

    $res = cashfreeRequest('/orders/' . rawurlencode($cfOrderId), null, 'GET');
    if (!$res || !isset($res['order_status'], $res['order_amount'])) jsonResponse(['success' => false, 'message' => 'Unable to verify payment'], 502);

    $expectedAmount = round((float)$order['total_amount'], 2);
    if (round((float)$res['order_amount'], 2) !== $expectedAmount) {
        ...
        if (is_array($payments)) {
            foreach ($payments as $p) {
                if (($p['payment_status'] ?? '') === 'SUCCESS') {
                    ...
                    $paymentAmountValid = isset($p['payment_amount']) && round((float)$p['payment_amount'], 2) === $expectedAmount;
                    break;
                }
            }
        }
        if ($paymentId === '' || !$paymentAmountValid) jsonResponse(['success' => false, 'message' => 'Payment transaction verification failed'], 409);

        $db->beginTransaction();
        try {
            $db->update("UPDATE orders SET payment_status = 'paid', cf_payment_id = ?, order_status = 'confirmed' WHERE id = ? AND payment_status = 'pending'", [$paymentId, $orderId], 'si');
    ...
    if ($res['order_status'] === 'ACTIVE') jsonResponse(['success' => false, 'message' => 'Payment still processing...', 'payment_status' => 'pending']);

    // Release reserved inventory only when the pending order transitions to failed.
    $affected = $db->update("UPDATE orders SET payment_status = 'failed' WHERE id = ? AND payment_status = 'pending'", [$orderId], 'i');
    if ($affected === 1) restoreReservedOrderStock($orderId);
...
if ($action === 'webhook') {
    ...
    if ($webhookData && isset($webhookData['data']['order']['order_id'])) {
        ...
        $paymentAmount = isset($webhookData['data']['payment']['payment_amount']) ? round((float)$webhookData['data']['payment']['payment_amount'], 2) : null;
        ...
        if ($order) {
            ...
            if ($payStatus === 'SUCCESS' && $paymentAmount === $expectedAmount && $cfPaymentId !== '') {
                $db->update("UPDATE orders SET payment_status = 'paid', cf_payment_id = ?, order_status = 'confirmed' WHERE id = ? AND payment_status <> 'paid'", [$cfPaymentId, $order['id']], 'si');
                $db->update("DELETE FROM cart_items WHERE user_id = ?", [$order['user_id']], 'i');
            } elseif (in_array($payStatus, ['FAILED', 'CANCELLED', 'VOID'], true)) {
                $affected = $db->update("UPDATE orders SET payment_status = 'failed' WHERE id = ? AND payment_status = 'pending'", [$order['id']], 'i');
                if ($affected === 1) restoreReservedOrderStock($order['id']);
            }
        }
    }
